...
$(document).ready(function(){	
    ...
    if($(this).val()!=""){
      ...
      if( !reg.test(chk_mail_val) ){
        ...
        $.ajax({
          ...
          url   : 'TS_webpage/mem_chk_member.php'
          ,type :'post'
  ...
  chk_pwd.bind("keyup",function(){
    ...
    function checkStrength(pwd){
      //假如密碼欄位內容值的長度小於6
      if(pwd.length<6){
        $('#msg_pwd').html('密碼未達6個字元<br>(應輸入6~20字元)！');
        test_pwd=false;	//設定pdTest變數為false否定(負責判斷密碼是否正確)
      }
      //假如密碼欄位內容值的長度大於20
      else if(pwd.length>20){
        $('#msg_pwd').html('密碼超過20個字元<br>(應輸入6~20字元)！');
        ...
        switch(strength){
          case 1:	//表示積分為1分時
            $('#msg_pwd').html(msg_blue_start+'密碼強度: '+msg_blue_end+m1+m0+m0+m0+m0);		break;
          case 2:	//表示積分為2分時
            $('#msg_pwd').html(msg_blue_start+'密碼強度: '+msg_blue_end+m1+m1+m0+m0+m0);		break;
          case 3:	//表示積分為3分時
            $('#msg_pwd').html(msg_blue_start+'密碼強度: '+msg_blue_end+m1+m1+m1+m0+m0);		break;
          case 4:	//表示積分為4分時
            $('#msg_pwd').html(msg_blue_start+'密碼強度: '+msg_blue_end+m1+m1+m1+m1+m0);		break;
          case 5:	//表示積分為5分時
            $('#msg_pwd').html(msg_blue_start+'密碼強度: '+msg_blue_end+m1+m1+m1+m1+m1);		break;
        }
      }
    }
  })
  ...
  chk_code.bind("keyup",function(){
    ...
    $.ajax(
      {	//要載入的是chkImgCode.php, 以POST方式傳入checkCode變數的值
