...
function fn_thumbnail($src_name, $path, $dst_name, $dst_w, $dst_h, $del_source = false)
{
    ...
    //準備影像 imagecreatefromjpeg(完整檔名) 返回一個來源圖像================
    switch ($src_ext) {
        case 1:$src_image = imagecreatefromgif($src_name);
            break;
        case 2:$src_image = imagecreatefromjpeg($src_name);
            break;
        case 3:$src_image = imagecreatefrompng($src_name);
            break;
    }
    ...
    //輸出圖像到指定位置指定檔名======================================
    switch ($src_ext) {
        case 1:imagegif($dst_image, $path . $dst_name);
            break;
        case 2:imagejpeg($dst_image, $path . $dst_name);
            break;
        case 3:imagepng($dst_image, $path . $dst_name);
            break;
    }
